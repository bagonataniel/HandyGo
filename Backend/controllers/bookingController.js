...
exports.createBooking = async (req, res) => {
    const [service_id, worker_id] = [req.body.service_id, req.body.worker_id];
    const client_id = jwt.decode(req.token).id;

    try {
        const result = await Booking.create(service_id, client_id, worker_id);
...
exports.getBookingDetails = async (req, res) => {
    ...
    const id = jwt.decode(req.token).id;
...
exports.getBookingsAsWorker = async (req, res) => {
    const worker_id = jwt.decode(req.token).id;
